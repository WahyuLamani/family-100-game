    ...
    const [visibleValues, setVisibleValues] = useState<{ [key: number]: string }>({});
      useEffect(() => {
          const handleKeyPress = (event: KeyboardEvent) => {
            const effect = question.surveys.find(quest => quest.id.toString() == event.key);
            if (event.key === 'x') {
            ...
            {question.surveys.map((survey) => {
                return (
                    <div key={survey.id} className='flex flex-row mb-4'>
                      <span className='text-1xl font-bold mt-[20px] mr-3'>{survey.id}</span>
                        <input type="text" disabled={true} defaultValue={visibleValues[survey.id] || ""} className="w-[560px] h-[60px] font-bold text-2xl bg-white text-center border-1 rounded-lg"/>
                        <span className={clsx('text-3xl text-green-500 font-bold mt-2 ml-5', {

  ...
  return (
    <div className="fixed bottom-4 left-4 flex space-x-4">
      {points.map((point, index) => {
        let teamName:{name:string,class:string} = index + 1 == 1 ? {name:"A",class:"bg-green-500"} : index + 1 == 2 ? {name:"B",class:"bg-yellow-500"} : {name:"C",class:"bg-red-500"};
        return (
            <div
              key={index}
              className={clsx("p-4 text-white text-2xl font-bold rounded-md w-50 text-center",teamName.class)}
            >
